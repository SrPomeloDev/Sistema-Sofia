...
class CamionDb(Base):
    ...
    ruta_madre: Mapped[str | None] = mapped_column(String(100), default="", nullable=True)
...
async def init_db():
    ...
    await _migrate_camiones()
    logger.info("Base de datos SQLite inicializada correctamente.")

async def _migrate_camiones():
    """Migraciones de schema para tabla camiones."""
    try:
        async with engine.begin() as conn:
            row = await conn.execute(
                text("SELECT COUNT(*) FROM pragma_table_info('camiones') WHERE name='ruta_madre'")
            )
            if row.scalar() == 0:
                await conn.execute(text("ALTER TABLE camiones ADD COLUMN ruta_madre VARCHAR(100) DEFAULT ''"))
                logger.info("Migración: columna ruta_madre agregada a camiones")
    except Exception as e:
        logger.warning("Migración de camiones saltada: %s", e)
    ...
    async with async_session_factory() as session:
        ...
        for item in camiones_list:
            ...
            if placa in placas_existentes:
                ...
                if "propietario" in item:
                    camion.propietario = item.get("propietario", "")
                if "ruta_madre" in item:
                    camion.ruta_madre = item.get("ruta_madre")
...
async def obtener_precio_promedio_ruta(ruta: str) -> float:
    """Busca el precio promedio para una ruta en la tabla de tarifas."""
    async with async_session_factory() as session:
        stmt = select(RutaTarifa).where(RutaTarifa.ruta == ruta)
        result = await session.execute(stmt)
        tarifa = result.scalar_one_or_none()
        return tarifa.precio if tarifa else 0.0

async def seed_promedios_desde_planilla(registros: list[dict]) -> int:
    """Guarda o actualiza promedios de ruta desde la planilla."""
    creados = 0
    async with async_session_factory() as session:
        for item in registros:
            ...
            descripcion = item.get("descripcion", f"Promedio desde planilla: {item.get('origen', '')}")
            stmt = select(RutaTarifa).where(RutaTarifa.ruta == ruta)
            result = await session.execute(stmt)
            ...
            if existente:
                ...
                session.add(RutaTarifa(ruta=ruta, precio=precio, sucursal=sucursal, descripcion=descripcion))
                creados += 1
        await session.commit()
    return creados

async def poblar_ruta_madre_desde_ruta() -> int:
    """Deduce ruta_madre desde ruta usando las rutas_madre del módulo Rutas. Re-procesa todos los camiones."""
    from modules.rutas.db.database import RutaMadreDb as RutasMadreDb
    from modules.rutas.db.database import async_session_factory as ruta_session

    async with ruta_session() as rs:
        r = await rs.execute(select(RutasMadreDb.nombre).distinct())
        nombres_validos = {row[0].strip().upper() for row in r if row[0]}
    ...
    async with async_session_factory() as session:
        ...
        for c in camiones:
            ruta = (c.ruta or "").strip()
            if not ruta:
                continue
            deducida = None
            if ruta.upper() in nombres_validos:
                deducida = ruta
            elif ruta.lower() == "local" and "MERCADO" in nombres_validos:
                deducida = "MERCADO"
            if deducida and c.ruta_madre != deducida:
                c.ruta_madre = deducida
                actualizados += 1
        await session.commit()
    return actualizados

async def asignar_rutas_desde_madres() -> int:
    """Asigna ruta desde ruta_madre usando las rutas_madre del módulo Rutas. Re-procesa todos los camiones."""
    from modules.rutas.db.database import RutaMadreDb as RutasMadreDb
    from modules.rutas.db.database import async_session_factory as ruta_session
    ...
    async with async_session_factory() as session:
        ...
        camiones = list(result.scalars().all())
        for c in camiones:
            madre = (c.ruta_madre or "").strip()
            if not madre:
                continue
            if madre.upper() in nombres_validos:
                nueva = "local" if madre.upper() == "MERCADO" else madre
                if c.ruta != nueva:
                    c.ruta = nueva
                    asignados += 1
        await session.commit()
